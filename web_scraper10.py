...
def save_to_db(result):
    ...
    try:
        ...
        if cursor:
            cursor.close()
        # Note: Connection is managed by teardown, so no need to close here
...
def save_news_analysis_to_db(result):
    ...
    try:
        ...
        if cursor:
            cursor.close()
        # Note: Connection is managed by teardown
...
# --- API Endpoint for Community Highlights Feature ---
@app.route("/api/community-threats")
def get_community_threats():
    """Fetches high-risk, recent threats from the DB to populate the highlights card."""
    threats = []
    cursor = None
    try:
        conn = get_db()
        ...
        query = """
            SELECT url, threat_score FROM site_analysis
            WHERE flag = 'malicious' AND threat_score >= 80
            ORDER BY analysis_timestamp DESC
            LIMIT 3
        """
        cursor.execute(query)
        threats = cursor.fetchall()
        # Rename 'threat_score' to 'score' to match frontend expectations
        for threat in threats:
            threat['score'] = threat.pop('threat_score')
    except mysql.connector.Error as err:
        print(f"DATABASE CONNECTOR ERROR: {err}")
        return jsonify({"error": "Failed to fetch community threats"}), 500
    except Exception as e:
        print(f"General Error in get_community_threats: {e}")
...
# --- API Endpoint for Feedback Feature ---
@app.route("/api/feedback", methods=['POST'])
def handle_feedback_api():
    """Saves user feedback on analysis accuracy to the database."""
    ...
    try:
        ...
        """)
        sql = "INSERT INTO analysis_feedback (url, is_accurate) VALUES (%s, %s)"
        cursor.execute(sql, (feedback_data['url'], feedback_data['accurate']))
        conn.commit()
    except Exception as err:
